...
test('redirects retain head metadata and a styled manual link', async () => {
  ...
  assert.match(output, /<section class="redirect-page" aria-labelledby="redirect-title">/)
  assert.match(output, /<h1 id="redirect-title">Redirecting…<\/h1>/)
  assert.match(output, /<a class="redirect-continue" href="\/resume\/\?a=1&amp;b=2">Continue to page/)
  assert.match(output, /<a class="redirect-home" href="\/">Back to home<\/a>/)
  assert.equal((output.match(/<html\b/g) ?? []).length, 1)
})
